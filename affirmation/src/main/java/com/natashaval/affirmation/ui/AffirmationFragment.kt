...
import com.natashaval.affirmation.R
import com.natashaval.affirmation.databinding.FragmentAffirmationBinding
import com.natashaval.affirmation.viewmodel.AffirmationViewModel
import com.natashaval.base.model.Status
import com.natashaval.base.utils.*
...
class AffirmationFragment : Fragment() {
  ...
  private fun createBigNotification(message: String?) {
    ...
    context?.let { ctx ->
      ...
      val notificationBuilder = NotificationCompat.Builder(ctx, CHANNEL_ID)
        ...
        .setContentText(message)
        .setSmallIcon(R.drawable.ic_hand_heart)
        .setLargeIcon(BitmapFactory.decodeResource(ctx.resources, R.drawable.ic_robot_love))
        .setStyle(NotificationCompat.BigTextStyle().bigText(message + message + message))
        .addAction(R.drawable.ic_heart, "Open Blibli", openBlibliIntent)
        .setContentIntent(viewPendingIntent)
        .extend(wearableExtender)
        .build()
  ...
  private fun createReplyNotification(message: String?) {
    ...
    context?.let { ctx ->
      ...
      val action = NotificationCompat.Action.Builder(R.drawable.ic_robot_love, getString(R.string.affirmation_reply), viewPendingIntent)
