...
import com.natashaval.pokedex.utils.Constant.Companion.POKEMON_ARTWORK_URL
...
class PokemonAdapter :
    PagingDataAdapter<NamedApiResource, PokemonAdapter.PokemonViewHolder>(DiffUtilCallback()) {
  ...
  inner class PokemonViewHolder(v: View) : RecyclerView.ViewHolder(v) {
    ...
    private fun buildArtworkUrl(url: String?): String? {
      ...
      val uri = Uri.parse(POKEMON_ARTWORK_URL).buildUpon().appendPath("$id.png").build().toString()
